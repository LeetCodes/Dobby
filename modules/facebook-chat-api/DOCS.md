...
If you want to be even more safe:  _login.js_ can get password with `require("readline")` or with environment letiables like this:
```js
let credentials = {
...
Returns current appState which can be saved to a file or stored in a letiable.
...
Sets letious configurable options for the api.
