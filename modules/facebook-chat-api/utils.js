...
function _formatAttachment(attachment1, attachment2) {
  ...
  let fileName = attachment1.filename
...
function formatDeltaMessage(m){
  ...
  let formatedMentions = [];
  if (m.delta.data && m.delta.data.prng) {
    let mentions = JSON.parse(m.delta.data.prng);
    mentions.map(function(elem) {
      formatedMentions.push({ userID: elem.i });
    });
    return {
      type: "message",
